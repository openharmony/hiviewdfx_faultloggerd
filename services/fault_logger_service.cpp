...
bool FaultLoggeExceptionReportService::Filter(int32_t connectionFd, const CrashDumpException& requestData)
{
    ...
    struct ucred creds{};
    if (!GetUcredByPeerCred(creds, connectionFd) || creds.uid != requestData.uid) {
...
#ifndef is_ohos_lite
int32_t FaultloggerdSdkDumpService::Filter(const std::string& socketName, const SdkDumpRequestData& requestData)
{
    if (requestData.pid <= 0 || socketName != SERVER_SDKDUMP_SOCKET_NAME) {
                                              ...
                                              const SdkDumpRequestData& requestData)
{
    ...
    int32_t responseCode = Filter(socketName, requestData);
