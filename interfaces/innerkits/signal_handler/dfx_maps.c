...
BOOL InitElfMaps(DfxElfMapsNode **head, pid_t pid)
{
    *head = (DfxElfMapsNode *)calloc(1, sizeof(DfxElfMapsNode));
    if ((*head) == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to create ElfMaps.");
        return FALSE;
    }

    char path[NAME_LEN] = {0};
    if (snprintf_s(path, sizeof(path), sizeof(path) - 1, "/proc/%d/maps", pid) <= 0) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to print path.");
        return FALSE;
    }

    char realPath[PATH_MAX] = {0};
    if (realpath(path, realPath) == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Maps path(%{public}s) is not exist.", path);
        return FALSE;
    }

    FILE *fp = fopen(realPath, "r");
    if (fp == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to open maps info.");
        free(*head);
        (*head) = NULL;
        return FALSE;
    }
    ...
    while (fgets(mapInfo, sizeof(mapInfo), fp) != NULL) {
        if (!InitElfMap(&map, mapInfo)) {
            HILOG_BASE_WARN(LOG_CORE, "Fail to init map info:%{public}s.", mapInfo);
            continue;
        }
...
BOOL InitElfMap(DfxElfMap **map, const char *mapInfo)
{
    *map = (DfxElfMap *)calloc(1, sizeof(DfxElfMap));
    if ((*map) == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to create ElfMap.");
        return FALSE;
    }
    ...
    if (sscanf_s(mapInfo, "%" SCNxPTR "-%" SCNxPTR " %4s %" SCNxPTR " %*x:%*x %*d%n", &((*map)->begin), &((*map)->end),
        ...
        &pos) != 4) { // 4:scan size
        HILOG_BASE_WARN(LOG_CORE, "Fail to parse maps info.");
        free(*map);
        (*map) = NULL;
        return FALSE;
    }
