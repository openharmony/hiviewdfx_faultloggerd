...
BOOL InitProcessWithKeyThread(DfxProcess **process, pid_t pid, DfxThread *keyThread)
{
    *process = calloc(1, sizeof(DfxProcess));
    if ((*process) == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to alloc process.");
        return FALSE;
    }
    ...
    if ((*process)->processName == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to alloc processName.");
        free(*process);
        *process = NULL;
        return FALSE;
    }
    ...
    if (!InitProcessMapsNode(*process)) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to init process maps.");
    ...
    if (!InitProcessThreadsNode(*process, keyThread)) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to init threads.");
    ...
    HILOG_BASE_INFO(LOG_CORE, "Init process dump with pid:%{public}d.", (*process)->pid);
    return TRUE;
}
