...
static const int CRASH_KERNEL_SNAPSHOT_FLAG = 8;
    ...
    if (pid < 0) {
        ...
        if (processDumpPid < 0) {
            ...
            if (endTime == 0 || endTime > curTime) {
                ...
                g_request->isUnwindSuccess = (intptr_t)&g_unwindSuccess;
                DFX_ExecDump();
            } else {
...
static void SetKernelSnapshot(bool enable)
{
    ...
    do {
        ...
        char *endPtr = NULL;
        int num = (int)strtol(val, &endPtr, 16); // 16 : val is hex
        if (*endPtr != '\0' && val == endPtr) {
            DFXLOGE("Failed to cast unexpecterd_die_catch val to int %{public}d", errno);
            break;
        }
        if (enable) {
            num |= CRASH_KERNEL_SNAPSHOT_FLAG;
        } else {
            num &= ~CRASH_KERNEL_SNAPSHOT_FLAG;
        }

        (void)memset_s(val, sizeof(val), 0, sizeof(val));
        if (snprintf_s(val, sizeof(val), sizeof(val) - 1, "%d", num) < 0) {
...
static void ReadUnwindFinishMsg(int signo)
{
    ...
    DFXLOGI("crash processdump unwind finish, unwind success Flag %{public}ld, blockFlag %{public}ld",
        g_unwindSuccess, g_blockFlag);
    if (g_unwindSuccess == CRASH_UNWIND_SUCCESS_FLAG) {
