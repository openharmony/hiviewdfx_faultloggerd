...
BOOL InitDfxElfImage(const char *path, DfxElfImage **image)
{
    ...
    if (realpath(path, realPath) == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to do realpath(%{public}s).", path);
        return FALSE;
    }

    *image = (DfxElfImage *)calloc(1, sizeof(DfxElfImage));
    if ((*image) == NULL) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to create ElfImage(%{public}s).", path);
        return FALSE;
    }

    (*image)->fd = open(realPath, O_RDONLY | O_CLOEXEC);
    if ((*image)->fd < 0) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to open elf file(%{public}s).", realPath);
        return FALSE;
    }

    struct stat elfStat;
    if (fstat((*image)->fd, &elfStat) != 0) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to get elf size.");
        close((*image)->fd);
        (*image)->fd = -1;
        return FALSE;
    }
    ...
    if (!ParseElfHeader((*image))) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to parse elf header.");
        close((*image)->fd);
        (*image)->fd = -1;
        return FALSE;
    }
    ...
    if (!ParseElfProgramHeader((*image))) {
        HILOG_BASE_WARN(LOG_CORE, "Fail to parse elf program header.");
        close((*image)->fd);
        (*image)->fd = -1;
        return FALSE;
    }
...
BOOL ParseElfHeader(DfxElfImage *image)
{
    ...
    if (nread < 0 || nread != sizeof(image->header)) {
        HILOG_BASE_WARN(LOG_CORE, "Failed to read elf header.");
        return FALSE;
    }
    return TRUE;
}
...
BOOL ParseElfProgramHeader(DfxElfImage *image)
{
    ...
    if (size > MAX_MAP_SIZE) {
        HILOG_BASE_WARN(LOG_CORE, "Exceed max mmap size.");
        return FALSE;
    }
    ...
    if (__builtin_add_overflow((uint64_t)size, (uint64_t)offset, &endOffset) ||
        ...
        HILOG_BASE_WARN(LOG_CORE, "Offset calculate error.");
        return FALSE;
    }
    ...
    if (map == MAP_FAILED) {
        HILOG_BASE_WARN(LOG_CORE, "Failed to mmap elf.");
        return FALSE;
    }
