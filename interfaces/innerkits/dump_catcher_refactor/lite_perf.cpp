...
private:
    ...
    bool HandlePollEvents(const struct pollfd (&readFds)[2], const int (&pipeFds)[2],
...
PerfErrorCode LitePerf::CollectProcessStackSampling(const LitePerfConfig& config, bool checkLimit, std::string& stacks)
{
    ...
    for (auto tid : config.tids) {
        ...
        int64_t jiffsToMs = DumpCatcherConstants::SECOND_TO_MS / sysconf(_SC_CLK_TCK);
...
PerfErrorCode LitePerf::Impl::StartProcessStackSampling(const LitePerfConfig& config, bool checkLimit)
{
    ...
    auto autoResetFlag = std::unique_ptr<std::atomic<bool>, void(*)(std::atomic<bool>*)>(&isRunning_,
        [] (std::atomic<bool>* flag) { flag->store(false); });
    enableDebugInfoSymbolic_ = config.parseMiniDebugInfo;
    int timeout = config.durationMs + DUMP_LITEPERF_TIMEOUT;
    int pipeReadFd[] = {-1, -1};
    int seconds = timeout / DumpCatcherConstants::SECOND_TO_MS;
    switch (RequestLitePerfPipeFd(FaultLoggerPipeType::PIPE_FD_READ, pipeReadFd, seconds, checkLimit)) {
        case ResponseCode::REQUEST_SUCCESS: {
            ...
            if (ExecDump(config.tids, config.freq, config.durationMs) < 0 ||
                DumpPoll(pipeReadFd, timeout) < 0) {
...
int LitePerf::Impl::DumpPoll(const int (&pipeFds)[2], const int timeout)
{
    ...
    do {
        ...
        int pRet = poll(readFds, 2, remainTime);
        if (pRet < 0) {
            if (errno == EINTR) {
                continue;
            }
...
bool LitePerf::Impl::InitDumpParam(const std::vector<int>& tids, int freq, int durationMs, LitePerfParam& lperf)
{
    ...
    if (DFX_SetDumpableState == nullptr || !DFX_SetDumpableState()) {
...
{
    for (int i = 0; i <= DumpCatcherConstants::WAITPID_RETRY_COUNT; i++) {
        ...
        usleep(DumpCatcherConstants::WAITPID_USLEEP_US);
    }
...
bool LitePerf::Impl::ExecDumpPipe(const int (&pipefd)[2], const LitePerfParam& lperf)
{
    ...
    char fdStr[DumpCatcherConstants::FD_STR_BUF_LEN];
...
int LitePerf::Impl::CollectSampleStackByTid(int tid, std::string& stack, int& count)
{
    ...
    std::unique_lock<std::mutex> lck(mutex_);
    auto stackIter = stackMaps_.find(tid);
    if (stackIter != stackMaps_.end()) {
        stack = stackIter->second.first;
        count = stackIter->second.second;
        if (!stack.empty()) {
            return 0;
        }
    }
