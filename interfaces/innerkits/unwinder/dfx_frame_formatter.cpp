...
std::string DfxFrameFormatter::GetFrameStr(const std::shared_ptr<DfxFrame>& frame)
{
    ...
    if (frame->isJsFrame) {
        ...
        data = StringPrintf("#%02d at %s (%s:%d:%d)", frame->index, frame->funcName.c_str(), frame->mapName.c_str(),
            frame->line, frame->column);
#endif
    } else {
#ifdef __LP64__
        data = StringPrintf("#%02d pc %016" PRIx64, frame->index, frame->relPc);
#else
        data = StringPrintf("#%02d pc %08" PRIx64, frame->index, frame->relPc);
#endif
        ...
        if (frame->funcName.length() > MAX_FUNC_NAME_LEN) {
            DFXLOG_WARN("length of funcName greater than 256 byte, do not display it");
