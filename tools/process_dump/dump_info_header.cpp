...
void DumpInfoHeader::Print(DfxProcess& process, const ProcessDumpRequest& request, Unwinder& unwinder)
{
...
#endif
        auto crashLogConfig = GetCrashLogConfig(request);
        process.SetCrashLogConfig(crashLogConfig);
        if (crashLogConfig.enabledCrashLogConfig) {
            headerInfo += "Enabled app log configs:\n";
            headerInfo += "Extend pc lr printing:";
            headerInfo += (crashLogConfig.extendPcLrPrinting ? "true\n" : "false\n");
            headerInfo += StringPrintf("Log cut off size:%" PRIu32 "B\n", crashLogConfig.logFileCutoffSizeBytes);
            headerInfo += "Simplify maps printing:";
            headerInfo += (crashLogConfig.simplifyVmaPrinting ? "true\n" : "false\n");
        }
    }
    ...
    if (request.type != ProcessDumpType::DUMP_TYPE_DUMP_CATCH) {
        ...
        headerInfo += StringPrintf("Process Memory(kB): %" PRIu64 "(Rss)\n",
            ReadProcRssMemInfo(process.GetProcessInfo().pid));
...
CrashLogConfig DumpInfoHeader::GetCrashLogConfig(const ProcessDumpRequest& request)
{
    ...
    if ((request.crashLogConfig & extendPcLrMask) == extendPcLrMask) {
        ...
        crashLogConfig.enabledCrashLogConfig = true;
    }
